...
g = Sequential()
g.add(Dense(z_dim*5, input_dim=z_dim,activation='relu'))
g.add(Dense(1000, activation='relu'))
g.add(BatchNormalization())
g.add(Dense(3000, activation='relu'))
g.add(Dense(6000, activation='relu'))
g.add(BatchNormalization())
g.add(Dense(9000, activation='relu'))
g.add(Dense(12000, activation='relu'))
g.add(Reshape((3000, 4)))
g.compile(loss=loss1, optimizer=adam, metrics=['accuracy'])
g.summary()


# d = Sequential()
# d.add(Dense(9000,activation='relu'))
# d.add(Dense(6000, activation='relu'))
# d.add(Dense(3000, activation='relu'))
# d.add(Dropout(0.5))
# d.add(Dense(1000, activation='relu'))
# d.add(Dense(112, activation='relu'))
# d.add(Dense(1, activation='relu'))
# d.compile(loss=loss1, optimizer=adam, metrics=['accuracy'])
# d.summary()
...
# g.add(Reshape((3000, 4)))
# g.add(LSTM(128, return_sequences=True))
# # g.add(UpSampling1D(size=2))
# g.add(LSTM(128, return_sequences=True))
# g.add(LSTM(4, return_sequences=True))
# g.compile(loss=loss1, optimizer=adam, metrics=['accuracy'])
# g.summary()

d = Sequential()
d.add(Conv1D(10,4,padding='same', input_shape=(3000, 4), activation='relu',kernel_initializer=tf.keras.initializers.RandomUniform()))
d.add(MaxPooling1D(2))
d.add(Conv1D(5,4,padding='same',activation='relu',kernel_initializer=tf.keras.initializers.RandomUniform()))
d.add(BatchNormalization())
d.add(MaxPooling1D(2))
d.add(Conv1D(4,4,padding='same',activation='relu',kernel_initializer=tf.keras.initializers.RandomUniform()))
d.add(Flatten())
d.add(Dropout(0.5))
...
gan.compile(loss= loss1, optimizer=adam, metrics=['accuracy'])
gan.summary()

# inputs = Input(shape=(z_dim, ))

# opt = Adam(lr=0.003,amsgrad = True)


# Gen_Model = MyGenerator()
# hidden = Gen_Model.build(inputs.shape)
# Gen_Model.summary()
# Gen_Model.compile(loss='binary_crossentropy', optimizer=opt, metrics=['accuracy'])

# inputs2 = Input(shape=(3000,4, ))
# Dis_Model = MyDiscriminator()
# output = Dis_Model.build(inputs2.shape)
# Dis_Model.summary()
# Dis_Model.compile(loss='binary_crossentropy', optimizer=opt, metrics=['accuracy'])

# gan = Sequential()
# gan = gan.add(Gen_Model)
# gan = gan.add(Dis_Model)
...
def train(epochs=10, plt_frq=1, BATCH_SIZE=10):
    ...
    for e in range(1, epochs+2):
        ...
        for _ in tqdm(range(batchCount)):  # tqdm_notebook(range(batchCount), leave=False):
            ...
            saveGeneratedSeq(generated_seq)
